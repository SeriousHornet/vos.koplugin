...
local _latest_ver = nil
local _dl_url = nil

local _plugin_dir = (debug.getinfo(1, "S").source or ""):match("^@?(.+)/[^/]+$")
    or "/mnt/onboard/.adds/koreader/plugins/vos.koplugin"
...
local function _currentVersion()
    ...
    if rok and type(rmeta) == "table" and rmeta.version then
        return rmeta.version
    end
    return "0.0.0"
end
...
local function _loadPersistedState()
    ...
    _dl_url = (type(url) == "string" and url:find("/releases/download/", 1, true)) and url or nil
...
local function _persistState(now)
    local gs = _gs()
    if not gs then
        return
    end
    if now then
        gs:saveSetting(GS_LAST_CHECK, now)
    end
...
local function _clearUpdateState()
    _has_update = false
    _latest_ver = nil
    _dl_url = nil
    local gs = _gs()
...
local function _httpGet(url)
    ...
    local code, headers, status = socket.skip(1, http.request({
        ...
        method = "GET",
        headers = {
            ["User-Agent"] = "KOReader-VOS/" .. _currentVersion(),
...
local function _httpGetToFile(url, dest_path)
    ...
    if not fh then
        return nil, "cannot create file: " .. tostring(err_open)
    end
    if ok_su then
    ...
    local code, headers, status = socket.skip(1, http.request({
        url = url,
        method = "GET",
        headers = { ["User-Agent"] = "KOReader-VOS/" .. _currentVersion() },
...
local function _parseRelease(body)
    ...
    if not ok_j then
        logger.warn("VOS updater: json module unavailable, using regex fallback")
...
local function _tmpZipPath()
    ...
    if ok and DS then
        ...
        if dir then
            candidates[#candidates + 1] = dir .. "/vos_update.zip"
        end
    end
    candidates[#candidates + 1] = "/tmp/vos_update.zip"
    candidates[#candidates + 1] = _plugin_dir .. "/vos_update.zip"
    ...
    return _plugin_dir .. "/vos_update.zip"
end
...
local function _applyUpdate(download_url, new_version)
    ...
    local progress_msg = _toast(
        string.format(_("Downloading VOS %s..."), new_version),
        120
    )
    ...
    local function doDownloadAndInstall()
        ...
        local uz_ok, uz_err = _unzip(tmp_zip, _parent_dir)
        os.remove(tmp_zip)
        if not uz_ok then
    ...
    local function handleInstallResult(result)
        ...
        if not result or not result.success then
            ...
            logger.err("VOS updater: failed in", stage, "-", err)
            _toast(
                stage == "download"
...
-- Install dialog with release notes

local function _showInstallDialog(release, current)
    ...
    if not _versionGt(latest, current) then
        _toast(string.format(_("VOS is up to date (%s)."), current))
        return
    end

    local header = string.format(
        _("VOS %s is available!\nYou have %s."),
        latest,
        current
    )
...
-- Network check with persistence
...
local function _doNetworkCheck()
    ...
    logger.info(string.format(
        "VOS updater: current=%s latest=%s has_update=%s",
        current,
        _latest_ver or "?",
        tostring(_has_update)
    ))
...
function M.buildBannerItem()
    ...
    return {
        ...
        keep_menu_open = true,
        callback = function()
            M.showInstallDialog(_currentVersion())
        end,
    }
end
...
function M.showInstallDialog(current_version)
    ...
    local function doCheck()
        ...
        local function handleCheckResult(release)
            ...
            if release.error then
                logger.err("VOS updater:", release.error)
                _toast(_("Error checking for updates: ") .. tostring(release.error))
                return
            end
            _latest_ver = release.version
            _dl_url = _isValidAssetUrl(release.download_url) and release.download_url or nil
            _has_update = _versionGt(_latest_ver, current_version)
            _persistState(os.time())
            _showInstallDialog(release, current_version)
        end
    ...
    if ok_nm and NetworkMgr and NetworkMgr.runWhenOnline then
        NetworkMgr:runWhenOnline(doCheck)
    else
        doCheck()
    end
end
...
function M.scheduleAutoCheck()
    ...
    UIManager:scheduleIn(5, function()
        ...
        local already_online = ok_nm and NetworkMgr
            and (
                (NetworkMgr.isOnline and NetworkMgr:isOnline())
